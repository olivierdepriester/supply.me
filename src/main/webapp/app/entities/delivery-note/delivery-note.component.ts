...
export class DeliveryNoteComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.deliveryNoteService
                ...
                .subscribe(
                    (res: HttpResponse<IDeliveryNote[]>) => this.paginateDeliveryNotes(res.body, res.headers),
    ...
    registerChangeInDeliveryNotes() {
        this.eventSubscriber = this.eventManager.subscribe('deliveryNoteListModification', response => this.reset());
    }
    ...
    private paginateDeliveryNotes(data: IDeliveryNote[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.deliveryNotes.push(data[i]);
        }
    }
